...
|---|---|---|
| `DOWNLOAD_ROOT` | `/downloads` | gallery-dl download output, organized by source/creator/work |
...
- **downloads/**: Original image files organized by `{source}/{creator_name}/{source_work_id}/`. Files are moved here from the flat gallery-dl output during import. No JSON metadata files kept — they are deleted after processing. No job_id layer in the path.
