...
| `503` | The deployment-level private-members foundation or Pixiv preview gate is closed, or the credential vault cannot load/authenticate the viewing account's stored credentials. No provider request was made. | Confirm the approved manual-preview rollout first. If both gates are open, verify the protected `REMOTE_CREDENTIAL_KEY` is present, valid, and is the same key that encrypted existing accounts; restore that key from protected rollback material rather than generating a replacement. Reconnect only an affected account when its ciphertext is damaged. Never print or copy keys, ciphertext, or decryptor diagnostics into logs/tickets. Keep automatic import and X/B discovery gates closed. |
