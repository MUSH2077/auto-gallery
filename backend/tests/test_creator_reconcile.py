...
async def test_relinks_single_creator_subscription():
    ...
    try:
        ...
        # Release the shared engine's pool so it does not stay pinned to this
        # test's event loop (which would break later integration tests that use
        # the same app engine, e.g. test_db_isolation).
