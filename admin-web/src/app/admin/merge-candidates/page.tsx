...
export default function MergeCandidatesPage() {
  ...
  return (
    <main className="max-w-4xl mx-auto p-6">
      ...
      {mc.isLoading && <div className="animate-pulse space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-16 bg-gray-100 rounded" />)}</div>}
      ...
      {mc.data?.candidates.map((c, i) => (
        <div key={i} className="bg-white rounded-lg shadow p-4 mb-2 text-sm">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-gray-500">{c.source_count} sources</span>
          </div>
