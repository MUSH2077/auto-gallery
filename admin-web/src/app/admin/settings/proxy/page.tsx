  ...
  return (
    <div className="mt-5">
      {proxy_reachable !== null && proxy_reachable !== undefined && (
        <div className={"mb-3 rounded-md border p-4 text-sm font-medium " + (proxy_reachable ? "border-[#dafbe1] bg-[#dafbe1] text-[#1a7f37] dark:border-[#238636]/30 dark:bg-[#238636]/15 dark:text-[#56d364]" : "border-[#ff8182]/40 bg-[#ffebe9] text-[#cf222e] dark:border-[#da3633]/40 dark:bg-[#da3633]/15 dark:text-[#ff7b72]")}>
          {proxy_reachable ? t("proxy.reachable") : t("proxy.unreachable") + " — " + proxy_reachable_error}
        </div>
      )}
      {proxy_reachable === false && (
        <div className="mb-3 rounded-md border border-[#fff8c5] bg-[#fff8c5] p-4 text-sm text-[#9a6700] dark:border-[#d29922]/30 dark:bg-[#d29922]/15 dark:text-[#f2cc60]">
            ...
            {results.map((r: any, i: number) => (
              <tr key={i} className="table-row">
                <td className="px-4 py-3">
                  ...
                  <div className="font-mono text-xs text-[#57606a] dark:text-[#8b949e]">{r.url}</div>
                </td>
                ...
                <td className="px-3 py-3">
                  {r.direct_error
                    ...
                    : r.direct_ok ? <span className="text-xs text-[#57606a] dark:text-[#8b949e]">—</span> : <span className="text-xs text-[#57606a] dark:text-[#8b949e]">{t("proxy.unknown_error")}</span>}
                </td>
                {proxyEnabled && (
                  <td className="text-center px-4 py-3">
                    {r.proxy_ok === null
                ...
                {proxyEnabled && (
                  <td className="px-3 py-3">
                    {r.proxy_error
                      ...
                      : r.proxy_ok ? <span className="text-xs text-[#57606a] dark:text-[#8b949e]">—</span> : <span className="text-xs text-[#57606a] dark:text-[#8b949e]">{t("proxy.unknown_error")}</span>}
                  </td>
                )}
              </tr>
            ))}
    ...
    <main className="max-w-4xl mx-auto p-6">
      ...
      {current && (
        <>
          ...
          <div className="mt-4 rounded-md border border-[#ddf4ff] bg-[#ddf4ff] p-4 text-sm text-[#0969da] dark:border-[#1f6feb]/30 dark:bg-[#1f6feb]/15 dark:text-[#58a6ff]">
