...
function ScheduledTimePicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="space-y-2">
      {times.length === 0 && <button onClick={addTime} className="px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-700 rounded hover:bg-slate-200 dark:hover:bg-slate-600">+ 添加时间</button>}
      {times.map((t, i) => (
  ...
  return (
    <main className="max-w-4xl mx-auto p-6">
      ...
      <div className="space-y-6">
        {/* ── Sync Schedule ── */}
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6 text-sm">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1"><input type="number" min={60} max={3600} step={60} value={dl.timeout_seconds}
                ...
                className="w-20 border rounded px-2 py-1 text-sm font-mono text-center" /><span className="text-xs text-gray-500">sec</span></div>
            </div>
            <div className="flex items-center justify-between">
