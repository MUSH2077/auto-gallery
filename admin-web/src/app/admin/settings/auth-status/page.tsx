...
export default function AuthStatusPage() {
  ...
  return (
    <main className="max-w-5xl mx-auto p-6">
      ...
      {auth.isLoading && (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-16 rounded-md bg-[#eaeef2] animate-pulse dark:bg-[#21262d]" />)}</div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            <div className="card p-4 text-center">
              ...
              <div className="text-xs text-[#57606a] dark:text-[#8b949e]">{t("auth.total")}</div>
            </div>
            <div className="card p-4 text-center">
              <div className="text-2xl font-bold text-green-600">{auth.data.summary.healthy}</div>
              <div className="text-xs text-[#57606a] dark:text-[#8b949e]">{t("auth.healthy")}</div>
            </div>
            <div className={`card p-4 text-center ${auth.data.summary.unhealthy > 0 ? "border-[#ff8182] dark:border-[#da3633]" : ""}`}>
              <div className={`text-2xl font-bold ${auth.data.summary.unhealthy > 0 ? "text-red-600" : ""}`}>{auth.data.summary.unhealthy}</div>
              <div className="text-xs text-[#57606a] dark:text-[#8b949e]">{t("auth.unhealthy")}</div>
            </div>
            <div className="card p-4 text-center">
              <div className="text-2xl font-bold text-[#57606a] dark:text-[#8b949e]">{auth.data.summary.unknown}</div>
              <div className="text-xs text-[#57606a] dark:text-[#8b949e]">{t("auth.unknown")}</div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="mt-0.5 font-mono text-xs text-[#57606a] dark:text-[#8b949e]">{s.source_url}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {s.auth_healthy === null && <span className="badge inline-flex items-center gap-1"><span className="w-2 h-2 bg-[#8b949e] rounded-full" /> {t("auth.unknown")}</span>}
                  {s.last_successful_auth && <span className="text-xs text-[#57606a] dark:text-[#8b949e]">{t("auth.last_success")} {new Date(s.last_successful_auth).toLocaleString()}</span>}
                </div>
              </div>
            </div>
          ))}

          <div className="mt-6 rounded-md border border-[#ddf4ff] bg-[#ddf4ff] p-4 text-sm text-[#0969da] dark:border-[#1f6feb]/30 dark:bg-[#1f6feb]/15 dark:text-[#58a6ff]">
            {t("auth.how")}
          </div>
        </>
      )}
