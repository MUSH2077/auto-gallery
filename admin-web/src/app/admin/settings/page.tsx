  ...
  return (
    <main className="max-w-4xl mx-auto p-6">
      ...
      <section className="mb-8">
        ...
        {settings.isLoading ? <div className="bg-white rounded-lg shadow p-4 animate-pulse"><div className="h-32 bg-gray-100 rounded" /></div> : (
          <div className="bg-white rounded-lg shadow p-4">
            <div className="text-sm space-y-3">
              {d && Object.entries(d).map(([key, value]) => (
                <div key={key} className="flex items-center justify-between py-2 border-b last:border-0">
                  <div><span className="font-medium">{key}</span>
                    <p className="text-xs text-gray-500 mt-0.5 max-w-md">{key === "source_level_enabled" ? "Same source + same ID = skip download" : key === "cross_source_enabled" ? "SHA-256 match across sources = reuse asset" : key === "auto_merge" ? "Auto-merge similar works without review. DANGER." : key === "phash_threshold" ? "Perceptual hash threshold (0-64). Lower = stricter." : ""}</p>
                  </div>
                  <span className={`px-2 py-0.5 rounded text-xs font-mono shrink-0 ml-4 ${typeof value === "boolean" ? (value ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500") : "bg-blue-100 text-blue-700"}`}>{String(value)}</span>
                </div>
              ))}
            </div>
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800"><strong>All deduplication is OFF by default.</strong> Auto-merge may irreversibly modify your library.</div>
          </div>
        )}
      </section>
