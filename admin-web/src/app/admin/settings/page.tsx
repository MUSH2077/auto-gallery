    ...
    <main className="mx-auto max-w-4xl p-6 page-transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        {cards.map((c) => (
          <Link key={c.href} href={c.href}
            ...
            <h2 className="mb-2 text-base font-semibold text-[#24292f] dark:text-ag-text">{c.title}</h2>
      ...
      <section className="mb-8">
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-[#24292f] dark:text-ag-text">{t("settings.language")}</p>
      ...
      <section className="mb-8">
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-[#24292f] dark:text-ag-text">{t("settings.reindex_label")}</p>
          ...
          <div className="card p-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              {Object.entries(settings.data.dedup || {}).map(([key, value]) => (
                <div key={key} className="flex justify-between border-b border-ag-border py-1 last:border-0 dark:border-ag-border">
      ...
      <section className="mb-8">
        ...
        <div className="card space-y-2 p-4 text-sm">
          <div className="flex justify-between"><span className="text-[#57606a] dark:text-[#8b949e]">{t("settings.backend_api")}</span><span className="font-mono text-xs text-[#24292f] dark:text-ag-text">{t("settings.backend_api_val")}</span></div>
          <div className="flex justify-between"><span className="text-[#57606a] dark:text-[#8b949e]">{t("settings.admin_web")}</span><span className="text-xs text-[#24292f] dark:text-ag-text">{t("settings.admin_web_val")}</span></div>
