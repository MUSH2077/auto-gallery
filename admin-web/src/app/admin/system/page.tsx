...
export default function SystemPage() {
  ...
  return (
    <main className="max-w-6xl mx-auto p-6">
      ...
      {health.error && <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg text-red-700">{(health.error as Error).message}</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
        {health.data ? Object.entries(health.data.services).map(([name, status]) => (
          ...
          Array.from({ length: 3 }).map((_, i) => <div key={i} className="bg-white rounded-lg shadow p-4 animate-pulse"><div className="h-4 bg-gray-200 rounded w-1/2 mb-2" /><div className="h-3 bg-gray-200 rounded w-3/4" /></div>)
        )}
      </div>
            ...
            <tbody>
              {sources.data?.sources?.map((s) => (
                <tr key={s.source_name} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{s.source_name}</td>
      ...
      {health.data && <p className="text-xs text-gray-400 mt-4">App version: {health.data.version} · Last update: {new Date().toLocaleTimeString()}</p>}
    </main>
  );
}
