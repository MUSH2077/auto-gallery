  ...
  return (
    <main className="max-w-6xl mx-auto p-6">
      ...
      <form onSubmit={(e) => { e.preventDefault(); setSubmitted(query); }} className="flex gap-2 mb-6">
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search works..." className="flex-1 border rounded px-4 py-2 text-sm dark:bg-slate-700 dark:text-white dark:border-slate-600" />
      ...
      {!submitted && <EmptyState title="Meilisearch" description="Enter a search term to find works by title, description, tags, or creator name." />}
      ...
      {results.data && !results.data.total && <EmptyState title="No results" description={`No results for "${submitted}"`} />}
      {results.data && results.data.total > 0 && (
        <div>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{results.data.total} result(s) for &quot;{submitted}&quot;</p>
            ...
            {results.data.results.map((r: any, i: number) => (
              <div key={r.id || i} className="bg-white dark:bg-slate-800 rounded-lg shadow p-4 flex gap-4 cursor-pointer hover:shadow-md transition-shadow" onClick={() => router.push(`/admin/works/${r.id}`)}>
                {r.thumbnail_asset_id ? (
                  ...
                  <div className="w-16 h-16 bg-gray-100 dark:bg-slate-700 rounded shrink-0 flex items-center justify-center text-gray-400 text-xs">N/A</div>
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm dark:text-white truncate">{r.title || "Untitled"}</span>
                    {r.is_nsfw && <span className="text-xs bg-red-100 dark:bg-red-900/30 text-red-600 px-1 rounded shrink-0">NSFW</span>}
                  ...
                  {r.tags && r.tags.length > 0 && (
                    <div className="flex gap-1 mt-1.5 flex-wrap">
                      {r.tags.slice(0, 8).map((t: string) => (
                        <span key={t} className="text-[10px] bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 px-1.5 py-0.5 rounded">{t}</span>
                      ))}
