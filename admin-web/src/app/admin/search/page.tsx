  ...
  return (
    <main className="max-w-4xl mx-auto p-6">
      ...
      <form onSubmit={(e) => { e.preventDefault(); setSubmitted(query); }} className="flex gap-2 mb-6">
        ...
        <button type="submit" className="px-6 py-2 bg-slate-900 text-white rounded text-sm hover:bg-slate-800">Search</button>
      </form>

      {results.isLoading && <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-16 bg-gray-100 rounded animate-pulse" />)}</div>}
      ...
      {results.data && results.data.total > 0 && (
        <div>
          <p className="text-sm text-gray-500 mb-3">{results.data.total} result(s) for &quot;{submitted}&quot;</p>
          <div className="space-y-2">
            {results.data.results.map((r: any, i: number) => (
              <div key={i} className="bg-white rounded-lg shadow p-4 text-sm">
