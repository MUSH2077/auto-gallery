...
type FilterMode = "all" | "active" | "inactive" | "has_danbooru" | "has_subscription" | "no_subscription";

const FILTERS: { key: FilterMode; label: string }[] = [
  { key: "all", label: "All" },
  { key: "active", label: "Active" },
  { key: "inactive", label: "Inactive" },
  { key: "has_danbooru", label: "Danbooru Linked" },
  { key: "has_subscription", label: "Subscribed" },
  { key: "no_subscription", label: "Not Subscribed" },
];
...
function CreateForm({ isPending, error, onSubmit, onClose }: {
  ...
  return (
    <div className="space-y-4">
      <div><label className="block text-sm font-medium mb-1">Name *</label><input value={name} onChange={(e) => setName(e.target.value)} className="w-full border rounded px-3 py-2 text-sm dark:bg-slate-700 dark:text-white" placeholder="Creator name" /></div>
      <div><label className="block text-sm font-medium mb-1">Display Name</label><input value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="w-full border rounded px-3 py-2 text-sm dark:bg-slate-700 dark:text-white" /></div>
      <div><label className="block text-sm font-medium mb-1">Description</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border rounded px-3 py-2 text-sm dark:bg-slate-700 dark:text-white" rows={3} /></div>
...
export default function CreatorsPage() {
  ...
  return (
    <main className="max-w-6xl mx-auto p-6">
      <PageHeader title="Creators" description={`${filtered.length} of ${creators.data?.length || 0} creators`}>
          ...
          {filtered.map((c) => (
            <div key={c.id} className={`bg-white dark:bg-slate-800 rounded-lg shadow-sm p-3 flex items-center gap-3 transition-colors ${selected.has(c.id) ? "ring-2 ring-blue-500" : ""}`}>
              ...
              <div className="flex items-center gap-2 shrink-0 text-xs">
                ...
                {subscriptionMap.has(c.id) && <span className="px-1.5 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded text-[10px]">sub</span>}
                <button onClick={() => router.push(`/admin/creators/${c.id}`)} className="text-blue-600 hover:underline">View</button>
                <button onClick={(e) => { e.stopPropagation(); setDeleteId(c.id); }} className="text-red-500 hover:text-red-700 dark:text-red-400">Del</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {deleteId && <ConfirmDialog open title="Delete Creator" message="This will permanently delete the creator and all associated links." onConfirm={() => del.mutate(deleteId)} onCancel={() => setDeleteId(null)} isPending={del.isPending} error={(del.error as Error)?.message} />}
      {confirmBatch && <ConfirmDialog open title="Batch Delete" message={`Delete ${selected.size} creators? This is irreversible.`} onConfirm={() => batchDel.mutate([...selected])} onCancel={() => setConfirmBatch(false)} isPending={batchDel.isPending} error={(batchDel.error as Error)?.message} />}
    </main>
  );
}
