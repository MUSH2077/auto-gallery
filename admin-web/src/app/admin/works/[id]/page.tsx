...
function AllPages({ workId, sources }: { workId: string; sources: WorkSourceData[] }) {
  ...
  if (!assets.data || !assets.data.length) return <div className="bg-white rounded-lg shadow p-4"><EmptyState title="No assets" description="No image files found for this work." /></div>;
    ...
    <div className="bg-white rounded-lg shadow p-4">
      ...
      {current && (
        <div className="mb-3">
          ...
          <p className="text-xs text-gray-400 mt-1 text-center">{activeIndex + 1} / {totalPages}</p>
        </div>
      )}
      ...
      {current && (
        <div className="mt-4 pt-3 border-t text-xs text-gray-500 space-y-1">
          <p className="font-medium text-gray-700 mb-1">Current Page</p>
    ...
    <main className="max-w-6xl mx-auto p-6">
        ...
        <PageHeader title={w.title || "Untitled"}
          description={
            <span className="flex items-center gap-3 flex-wrap">
              ...
              {rating && <span className="text-xs px-2 py-0.5 rounded bg-gray-100">{rating}</span>}
              {illustType && <span className="text-xs px-2 py-0.5 rounded bg-purple-100 text-purple-700">{illustType}</span>}
              {w.is_nsfw && <span className="text-xs px-2 py-0.5 rounded bg-red-100 text-red-700">NSFW</span>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
          {/* Summary Card */}
          <div className="bg-white rounded-lg shadow p-4">
            <h3 className="font-medium mb-3 text-sm uppercase tracking-wide text-gray-400">Summary</h3>
            <dl className="text-sm space-y-2">
              <div className="flex gap-2"><dt className="text-gray-500 w-24 shrink-0">Title:</dt><dd className="font-medium">{w.title || "Untitled"}</dd></div>
              {primaryWs?.source_work_id && (
                <div className="flex gap-2"><dt className="text-gray-500 w-24 shrink-0">Work ID:</dt><dd className="font-mono text-xs">{primaryWs.source_work_id}</dd></div>
              )}
              <div className="flex gap-2"><dt className="text-gray-500 w-24 shrink-0">Posted:</dt><dd>{createDate ? new Date(createDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : "Unknown"}</dd></div>
              {rawWidth && rawHeight && (
                <div className="flex gap-2"><dt className="text-gray-500 w-24 shrink-0">Dimensions:</dt><dd>{rawWidth} &times; {rawHeight}</dd></div>
              )}
              <div className="flex gap-2"><dt className="text-gray-500 w-24 shrink-0">Pages:</dt><dd>{pageCount}</dd></div>
              {illustType && (
                <div className="flex gap-2"><dt className="text-gray-500 w-24 shrink-0">Type:</dt><dd className="capitalize">{illustType}</dd></div>
              )}
              <div className="flex gap-2"><dt className="text-gray-500 w-24 shrink-0">Imported:</dt><dd className="text-xs">{new Date(w.created_at).toLocaleString()}</dd></div>
            </dl>
          </div>
          ...
          {hasStats && (
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-medium mb-3 text-sm uppercase tracking-wide text-gray-400">Stats</h3>
          ...
          {(rawTags.length > 0 || (workTags.data && workTags.data.length > 0)) && (
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-medium mb-3 text-sm uppercase tracking-wide text-gray-400">
              ...
              {workTags.data && workTags.data.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {workTags.data.map((t) => (
                    <span key={t.id} className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm border border-blue-200"
              ...
              {rawTags.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {rawTags.map((tag, i) => (
                    <span key={i} className="px-3 py-1 bg-gray-100 rounded-full text-sm hover:bg-gray-200 cursor-default"
          ...
          {rawTools.length > 0 && (
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-medium mb-3 text-sm uppercase tracking-wide text-gray-400">Tools</h3>
              <div className="flex flex-wrap gap-2">
                {rawTools.map((tool, i) => (
          ...
          {w.description && (
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-medium mb-2 text-sm uppercase tracking-wide text-gray-400">Description</h3>
        ...
        <div className="space-y-4">
          ...
          {series && (
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-medium mb-2 text-sm text-gray-500">Series</h3>
          ...
          {primaryWs?.source_url && (
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-medium mb-2 text-sm text-gray-500">Links</h3>
  ...
  return (
    <div className="border rounded-lg p-3 text-sm">
      <div className="flex items-center gap-2 mb-2">
        ...
        {s.source_creator_id && (
          <span className="text-xs text-gray-400">by {s.source_creator_id}</span>
        )}
      </div>
      {s.source_url && (
      ...
      {s.title && <p className="text-xs text-gray-600 mt-1">Source title: {s.title}</p>}
      {s.posted_at && <p className="text-xs text-gray-400 mt-1">Posted: {s.posted_at}</p>}
      {s.raw_metadata && (
        <div className="mt-2">
          ...
          {showRaw && (
            <pre className="mt-2 text-xs font-mono bg-gray-50 p-3 rounded max-h-64 overflow-auto">
